...
// A viewer attaching reads the track codec for its muxer while another
// consumer's depacketizer updates the same codec's FmtpLine. Meaningful under -race.
func TestAddTrackWhileFmtpUpdates(t *testing.T) {
	...
	for range 100 {
		...
		_ = cons.muxer.GetInit() // what WriteTo sends first, built from the shared codec
		_ = cons.Stop()
	}
